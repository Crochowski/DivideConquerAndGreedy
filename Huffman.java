...
public class Huffman {
    ...
    private static BinaryTrie findOptimalCode(HeapOfBinaryTries H) {
        // TASK 3.B
        BinaryTrie z = null;        // Initialise node to store combined x + y frequencies

        while (H.size() > 1) {      // Until there is only 1 node in the heap

            BinaryTrie x = H.extractMin();  // Extract the two nodes with the lowest frequency
            BinaryTrie y = H.extractMin();

            z = new BinaryTrie(x, y);       // Create a new node to store the frequency x + y

            H.insert(z);                    // Insert the new node z back in the heap

        }
        return z;                           // Return the single node after all combinations complete
    }
